...
async function runAmplifierChain(program, phases, feedbackMode) {
  ...
  if (feedbackMode) {
    ...
    for (let i = 0; i < phases.length; i++) {
      ...
      amplifiers[i].inputGenerator = ampInputGenerator(
        amplifiers[prevAmp],
        phases[i],
        i == 0
      );
    ...
    for (let i = 0; i < phases.length; i++) {
      let amp = new IntCode(
        program,
        arrayInputGenerator([phases[i], nextInput])
      );
      await amp.run();
...
async function findMaxPhaseSetting(program, feedbackMode) {
  ...
  for (const permutation of findPerms(phaseValues)) {
    ...
    let out = parseInt(
      await runAmplifierChain(program, permutation, feedbackMode)
    );
    if (out > maxSoFar) {
  ...
  read_text_input("inputs/day7.txt", (input) => {
    ...
    Promise.all([
      ...
      document.getElementById(
        "day7"
      ).innerHTML = `Max once: ${result[0][1]} (${result[0][0]}) Max w feedback: ${result[1][1]} (${result[1][0]})`;
    });
  });
}
...
function tests_day7() {
  Promise.all([
    findMaxPhaseSetting(
      ...
      false
    ).then((res) => test_assert("1.1", res[1], 43210)),
    findMaxPhaseSetting(
      [
        3,
        23,
      ...
      false
    ).then((res) => test_assert("1.2", res[1], 54321)),
    findMaxPhaseSetting(
      [
        3,
        31,
      ...
      false
    ).then((res) => test_assert("1.3", res[1], 65210)),
    findMaxPhaseSetting(
      [
        3,
        26,
      ...
      true
    ).then((res) => test_assert("2.1", res[1], 139629729)),
    findMaxPhaseSetting(
      [
        3,
        52,
      ...
      true
    ).then((res) => test_assert("2.2", res[1], 18216)),
  ]).then((results) => {
